...
keys <- list(
  "measuring unit" = c("cm", "km", "m", "milla", "millas", "kg", "g", "t", "tonelada", "toneladas"), ### SPANISH ###
  "area" = c("ha", "hectárea", "hectáreas", "km2", "acre", "acres"), ### SPANISH ###
  "temperature" = c("ºc", "oc", "ºf"), 
  "ghg" = c("mt", "mtco2e", "mtco₂e", "co2", "co2e", "co2eq", "co2-eq", "ch4"),
  "energy" = c("mw", "watt", "watts", "w", "vatio", "vatios"), ### SPANISH ###
  "money" = c("dólar", "dólares", "usd", "euro", "euros", "eur", "$", "£", "¥", "€"), ### SPANISH ###
  "percentage change" = c("%", "porciento", "porcentaje") ### SPANISH ###
)

quant_fills <- function(quant_df) {
  if (nrow(quant_df) == 0) {
    txt_quant_fills <- "no se refiren a ningun elemento" ### SPANISH ###
  } else {
    quant_df <- quant_df %>% 
      ...
      if (length(quant_tbl[quant_tbl >= mean(quant_tbl)]) == 1) { # (mean(quant_tbl)+sd(quant_tbl))
        ...
        for (r in 1:length(keyword_lookup)) {
          txt_quant_fills <- paste0(txt_quant_fills, keyword_lookup[r], " ('", names(keyword_lookup)[r], "')", 
                                    ifelse(r == length(keyword_lookup)-1, ", and ", ", ")) ### SPANISH ###
        }
...
temps_fills <- function(temp_df) {
  if (nrow(temp_df) == 0) {
    txt_temp_fills <- "no se refiren a ningun elemento" ### SPANISH ###
  } else {
    temp_df <- temp_df %>% 
...
En total, el `r round(100*sum(count_qt$quants)/dim(tgts)[1],0)`% de las `r dim(tgts)[1]` metas parecen ser cuantitativas (`r txt_qtq`), lo que significa que estas metas pueden ser más específicas y medibles que otras. De las metas cuantitativos, las Metas Nacionales de Biodiversidad `r fill_nat_qt`. Las metas de las CDN `r fill_cli_qt`. Por último, las que pertenecen a las otros metas `r fill_oth_qt`.

<br>

Además, el `r round(100*sum(count_qt$dates)/dim(tgts)[1],0)`% de las `r dim(tgts)[1]` parecen estar sujetos a plazos (`r txt_qtt`). De las metas sujetos a plazos, las Metas Nacionales de Biodiversidad `r fill_nat_tt`, mientras que las metas de las CDN `r fill_cli_tt`. Por último, las otras metas `r fill_oth_tt`.
:::
...
::: {custom-style="Text"}
Las categorías de soluciones basadas en la naturaleza que aparecen más referenciadas en las metas son `r aux1$Theme[dim(aux1)[1]]` y `r aux1$Theme[dim(aux1)[1]-1]`. Por otro lado, las categorías de soluciones basadas en la naturaleza que aparecen menos referenciadas son `r aux1$Theme[1]` y `r aux1$Theme[2]`. En la [`r ftext("Guía para la inclusión de la naturaleza en las Contribuciones Determinadas a Nivel Nacional se la Guía de para la inclusión de la naturaleza en las Contribuciones Determinadas a Nivel Nacional", links)`](https://nature4climate.org/wp-content/uploads/2024/11/N4C-Guide-Nature-NDCs.pdf) de Nature4Climate pueden encontrar perguntas para tener en cuenta a la hora de revisar los resultados.

<br>

*En la [`r ftext("encuesta", links)`](https://forms.office.com/Pages/ResponsePage.aspx?id=Xtvls0QpN0iZ9XSIrOVDGWNp7QxCnxtBnoa-dEHQqQxUMlIxV0FOSzdWTkFCMUJFTFFFMFc4UFNURy4u), se pide a los países que informen sobre si la evaluación es demasiada generosa o restrictiva en determinados ámbitos.*
:::
...
theme_chunks <- function(theme, tmp_th, tmp_tt, summ, dir = "chunks", graph_path) {
  ...
  writeLines(glue("
                  ...
                  *{filter(thms, Theme == theme)$Texts}*
                  ...
                  ::: {{custom-style='Text'}}
                  El modelo de IA identificó {ifelse(dim(tmp_th)[1] == 0, 'ninguna', if (dim(tmp_th)[1] < 10) {as.character(as.english(dim(tmp_th)[1]))} else {dim(tmp_th)[1]})} meta que podría relacionarse con este tema de {rmd_fill}:
                  :::
...
tab_cnt <- 0
#tab_list <- list()
rmd_fill <- 'solicón baseada en la naturaleza' ### SPANISH ###
...
for (theme in filter(thms, Area == 'NbS')$Theme) {
  ...
  if (dim(tmp_tt)[1] == 0) {
    txt_tt1 <- paste0('ningun par muestra') ### SPANISH ###
    txt_tt2 <- '.'
...
::: {custom-style="Text"}
En total, el `r round(100*sum(count_qt$quants)/dim(tgts)[1],0)`% de las `r dim(tgts)[1]` metas parecen ser cuantitativas (`r txt_qtq`), lo que significa que estas metas pueden ser más específicas y medibles que otras. De los objetivoslas metas cuantitativos, los que pertenecen a los Objetivos Nacionales de Biodiversidad `r fill_nat_qt`. Las que pertenecen a las metas de las Contribuciones Determinadas a Nivel Nacional `r fill_cli_qt`. Por último, los demás objetivos están relacionados con las otras metas `r fill_oth_qt`.
...
::: {custom-style="Text"}
Las metas identificados como cuantitativos para `r artgen``r params$country` incluyen:  
:::
...
::: {custom-style="Text"}
Las metas identificados como sujetos a plazo para `r artgen``r params$country` incluyen:  
:::
...
::: {custom-style="bullet-point"}
- Representación de **Soluciones basadas en la naturaleza (SbN)**
